<script lang="ts">
  ...
  const getBookmarks = async () => {
    ...
    const ev10002: NostrEvent | undefined = await fetcher.fetchLastEvent(Array.from(relaySet), { kinds: [10002], authors: [targetPubkey] });
    ...
    const ev3: NostrEvent | undefined = await fetcher.fetchLastEvent(relays, { kinds: [3], authors: [targetPubkey] });
    if (ev3 === undefined) {
  ...
  const setBookmarkedPubkeys = async (
    ...
    bookmarkedPubkeys: Map<string, Set<string>>,
  ): Promise<[Set<string>, Map<string, Set<string>>]> => {
    ...
    {#each sortedEvents as note}
      ...
      {#if count >= threshold}
        ...
        <dt>
          ...
          >
          <a
            href="{linkto}{note.kind === 1 ? nip19.noteEncode(note.id) : nip19.neventEncode({ ...note, author: note.pubkey })}"
            target="_blank"
            rel="noopener noreferrer"><br /><time>{new Date(1000 * note.created_at).toLocaleString()}</time></a
          ><span
